...
<div class="row">
    <div class="col col-lg-8">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('event.avl.update', $event->id) }}" method="post" enctype="multipart/form-data">
                    @method('patch')
                    @csrf
                    ...
                    <div class="form-group form-check">
                        <input value="true" type="checkbox" class="form-check-input" id="avlb" name="available" @if($avail->available) checked @endif onchange="changeAvlb()">
                        <label class="form-check-label" for="avlb">Available</label>
                    </div>

                    <div class="row form-group mb-3 @if(!$avail->available) d-none @endif" id="time" >
                        <div class="col">
                            
                            <label for="start_at">Start</label>
                            <input id="start_at" class="form-control @error('start_at') is-invalid @enderror" type="text" name="start_at" value="@if(old('start_at')){{ old('start_at') }}@else @if($avail->available){{Carbon\Carbon::parse($avail->start)->format("H:i\z")}}@else{{Carbon\Carbon::parse($event->start)->format("H:i\z")}}@endif @endif" required >
                            @error('start_at')
                        ...
                        <div class="col">
                            ...
                            <input id="end_at" class="form-control @error('end_at') is-invalid @enderror" type="text" name="end_at" value="@if(old('end_at')) {{ old('end_at') }} @else @if($avail->available) {{Carbon\Carbon::parse($avail->end)->format("H:i\z")}} @else {{Carbon\Carbon::parse($event->end)->format("H:i\z")}} @endif @endif" required >
                            @error('end_at')
...
window.addEventListener('load', function(){
    console.log(document.getElementById('start_at').value.trim());
    document.getElementById('start_at').value = document.getElementById('start_at').value.trim();
    document.getElementById('end_at').value = document.getElementById('end_at').value.trim();

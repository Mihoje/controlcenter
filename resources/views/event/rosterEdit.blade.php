...
<div class="row">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                    ...
                    <div>
                        ...
                        <div class="col"><input type="checkbox" id="change{{$station->id}}" name="stationsCheck[]" value="{{$station->id}}" onchange="addPos({{$station->id}})" @if(array_key_exists($station->callsign, $roster_stations)) checked @endif/>{{$station->callsign}}</div>
    ...
    <div class="col col-lg-7">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <form action="{{route("event.roster.edit.save")}}" method="post" id="myform" name="myform" class="myform" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="d-none">
                        ...
                        @if($inner%4==1) </div> @endif
                        @if($i%4==0) <div class="row"> @endif
                        <div class="col"><input type="checkbox" hidden id="change2{{$station->id}}" name="stationsCheck[]" value="{{$station->id}}" onchange="addPos({{$station->id}})" @if(array_key_exists($station->callsign, $roster_stations))  checked @endif/></div>
                    ...
                    <div class="border-bottom "  @if(!array_key_exists($station->callsign, $roster_stations)) style="display: none;" @endif id="station{{$station->id}}Cont" >
                    ...
                    @if(isset($roster_stations[$station->callsign]))
                    @php $i=1; @endphp
                    @foreach($roster_stations[$station->callsign] as $ro)
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                ...
                                <select class="form-control selectCont" name="controllers{{$station->id}}{{$i}}[]" id="controllers{{$station->id}}{{$i}}[]" multiple >                               
                                     @foreach($avlb as $av)       
                                        <option value="{{$av->user_id}}" @if(in_array($av->user_id, $ro['user'])) selected @endif>{{$user_arr[$av->user_id]}}</option>
                                    @endforeach
                                  </select>
                            </div>
                        </div>
                        ...
                        <div class="col">
                            ...
                            <label for="start_at{{$station->id}}{{$i}}">Start (Zulu)</label>
                            <input id="start_at{{$station->id}}{{$i}}" class="form-control start_time @error('start_at') is-invalid @enderror" type="time" name="start_at{{$station->id}}{{$i}}" placeholder="{{trim(Carbon\Carbon::parse($event->start)->format("H:i"))}}" value="@if(old('start_at')){{trim(old('start_at'))}}@else{{trim($ro['start'])}}@endif" required onChange="checkTime()">
                            @error('start_at')
                        ...
                        <div class="col">
                            ...
                            <input id="end_at{{$station->id}}{{$i}}" class="form-control end_time @error('end_at') is-invalid @enderror" type="time" name="end_at{{$station->id}}{{$i}}" placeholder="{{trim(Carbon\Carbon::parse($event->end)->format("H:i"))}}" value="@if(old('end_at')){{trim(old('end_at'))}}@else{{trim($ro['end'])}}@endif" required onChange="checkTime()">
                            @error('end_at')
                        ...
                        <div class="col" >
                            <div class="form-group">
                                ...
                                <select class="form-control selectCont" name="mentors{{$station->id}}{{$i}}[]" id="mentors{{$station->id}}{{$i++}}[]" multiple >                               
                                     @foreach($avlb as $av)       
                                        <option value="{{$av->user_id}}" @if($ro['mentors'] && in_array($av->user_id, $ro['mentors'])) selected @endif>{{$user_arr[$av->user_id]}}</option>
                                    @endforeach
                                  </select>
                            </div>
                        </div>
                    ...
                    <div class="border-bottom "   id="statioBackupCont" >
                            ...
                            <div class="col">
                                <div class="form-group">
                                    ...
                                    <select class="form-control selectCont" name="controllersBackup[]" id="controllersBackup[]" multiple >                               
                                         @foreach($avlb as $av)       
                                            <option value="{{$av->user_id}}" @if(isset($backup[0]) && in_array($av->user_id, $backup[0])) selected @endif>{{$user_arr[$av->user_id]}}</option>
                                        @endforeach
                                      </select>
                                </div>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="" name="msg" id="msg" onchange="updateMsg()">{!!$event->roster_notes!!}</textarea>
                            <script>
                    ...
                    <a class="btn btn-success mt-1" href="{{route('event.roster.publish', $event->id)}}" >Publish roster</a>
                </form>
            </div>
        </div>
    </div>
